...
export const activitiesApi = {
  ...
  retroRecalc: async (id: string, from_date: string): Promise<{ replaced: number; refunded: number }> => {
    const { data } = await apiClient.post<{ replaced: number; refunded: number }>(`/activities/${id}/retro-recalc`, { from_date })
    return data
  },
}
