...
function SalaryPaymentRow({ payment, isOwner, accounts, onRefresh, view = 'row' }: {
  payment: SalaryPayment
  isOwner: boolean
  accounts: { id: string; name: string }[]
  onRefresh: () => void
  ...
  if (view === 'card') {
    return (
      <div className={`bg-white rounded-xl shadow-sm border border-gray-200 p-4 flex flex-col gap-3 ${payment.is_dividend ? 'border-purple-200 bg-purple-50/30' : ''}`}>
        <div className="flex justify-between items-start gap-2">
          <div>
            <div className="font-medium text-gray-900 text-sm mb-1">
              {payment.staff_name}
              {payment.is_dividend && (
                <span className="ml-2 text-[10px] bg-purple-100 text-purple-700 px-1.5 py-0.5 rounded font-medium">дивіденд</span>
              )}
            </div>
            <div className="text-xs text-gray-500">{String(payment.transaction_date).slice(0, 10)} • {payment.account_name ?? '—'}</div>
          </div>
          <div className="text-right">
            <div className="font-mono font-semibold text-gray-900">{fmt(payment.gross_amount)}</div>
          </div>
        </div>
            ...
            <button
              ...
              className={`text-xs px-2 py-1 rounded border transition-colors ${
                payment.is_dividend
                  ...
                  : 'text-gray-500 hover:text-purple-600 hover:bg-purple-50 border-gray-200'
              }`}
          ...
          {salaryLoading ? (
            ...
            <>
              ...
              <div className="md:hidden bg-gray-50 border-t border-gray-200 p-4 space-y-2 text-sm">
                <div className="flex justify-between font-medium text-gray-700">
                  <span>Разом</span>
                  <span className="font-mono">{salaryTotal.toFixed(2)}</span>
                </div>
              </div>
            </>
          )}
