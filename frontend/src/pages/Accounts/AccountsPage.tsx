...
export function AccountsPage() {
  ...
  const createMutation = useMutation({
    ...
    onSuccess: () => { qc.invalidateQueries({ queryKey: ['accounts'] }); setAddingNew(false); setNewForm({ name: '', type: 'fop', currency: 'UAH', note: '' }); setNewError(null) },
    ...
    <div className="max-w-2xl space-y-6">
      <div className="flex items-center justify-between">
        <div>
          ...
          <p className="text-sm text-gray-500">{activeCount} активних{inactiveCount > 0 ? `, ${inactiveCount} архів` : ''}</p>
        </div>
        {canEdit && !addingNew && (
          <button onClick={() => setAddingNew(true)} className="px-4 py-2 bg-iris-600 hover:bg-iris-700 text-white text-sm font-medium rounded-lg transition-colors">
            + Додати рахунок
          </button>
        )}
      </div>
                    ...
                    <>
                      <td className="px-4 py-2">
                        <input type="text" value={editForm.name} onChange={(e) => setEditForm({ ...editForm, name: e.target.value })}
                          autoFocus className="w-full rounded border-gray-300 text-sm shadow-sm focus:border-iris-500 focus:ring-iris-500" />
                      </td>
                      <td className="px-4 py-2">
                        <select value={editForm.type} onChange={(e) => setEditForm({ ...editForm, type: e.target.value as Account['type'] })}
                      ...
                      <td className="px-4 py-2 hidden sm:table-cell">
                        <input type="text" value={editForm.currency} onChange={(e) => setEditForm({ ...editForm, currency: e.target.value })}
                          className="w-20 rounded border-gray-300 text-sm shadow-sm focus:border-iris-500 focus:ring-iris-500" />
                      </td>
                      <td className="px-4 py-2" />
                      <td className="px-4 py-2">
                        <div className="flex items-center gap-2 justify-end">
                          <button onClick={() => updateMutation.mutate({ id: account.id, ...editForm })} disabled={updateMutation.isPending}
                          ...
                          <button onClick={() => setEditingId(null)} className="text-xs px-2 py-1 text-gray-500 hover:text-gray-900">Скасувати</button>
                        </div>
                      </td>
                    </>
                  ) : (
                      ...
                      {canEdit && (
                        <td className="px-4 py-3">
                          <div className="flex items-center gap-3 justify-end">
                            <button onClick={() => startEdit(account)} className="text-xs text-iris-600 hover:text-iris-700 font-medium">Редагувати</button>
                            <button onClick={() => toggleActiveMutation.mutate({ id: account.id, is_active: !account.is_active })}
                ...
                <tr className="bg-iris-50">
                  <td className="px-4 py-2">
                    <div className="space-y-1">
                      <input type="text" value={newForm.name} placeholder="Назва рахунку" autoFocus
                  ...
                  <td className="px-4 py-2">
                    <select value={newForm.type} onChange={(e) => setNewForm({ ...newForm, type: e.target.value as Account['type'] })}
                      className="rounded border-gray-300 text-sm shadow-sm focus:border-iris-500 focus:ring-iris-500">
                      <option value="fop">ФОП</option>
                      <option value="cash">Готівка</option>
                      <option value="bank">Банк</option>
                  ...
                  <td className="px-4 py-2 hidden sm:table-cell">
                    <input type="text" value={newForm.currency} onChange={(e) => setNewForm({ ...newForm, currency: e.target.value })}
                      className="w-20 rounded border-gray-300 text-sm shadow-sm focus:border-iris-500 focus:ring-iris-500" />
                  </td>
                  <td className="px-4 py-2" />
                  <td className="px-4 py-2">
                    <div className="flex items-center gap-2 justify-end">
                      ...
                      <button onClick={() => { setAddingNew(false); setNewError(null) }} className="text-xs px-2 py-1 text-gray-500 hover:text-gray-900">Скасувати</button>
                    </div>
                  </td>
                </tr>
              )}
